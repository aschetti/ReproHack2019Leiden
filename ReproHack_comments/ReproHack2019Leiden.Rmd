...
Below is the original script (file available at `r here("authors_materials_OSF/Analysis/R Script PDR1.R")`).
...
* load the data
    ...
    * better to integrate the codebook (available at `here("authors_materials_OSF/Codebook PDR1.pdf")`) within the analysis script, to help replicators understand the data (see the R package [`codebook`](https://github.com/rubenarslan/codebook))
...
* if exploratory analyses are run with and without outliers, save the two datasets in separate variables
...
- **R Script PDR1.R**
    - follow code conventions (e.g., be consistent when using either `=` or `<-` when assigning variables): see the package `styler`
